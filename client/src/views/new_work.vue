<template>
  <div class="new-work">
    ...
    <table v-if="openWorks.length">
      ...
      <tbody>
        <tr v-for="(work, index) in openWorks" :key="index">
          ...
          <td>{{ work.description }}</td>
          <td>
            <button @click="editWorkMethod(work)">Edit</button>
          </td>
        </tr>
      </tbody>
    </table>
    ...
    <form @submit.prevent="submitNewWork">
      ...
      <select id="city" v-model="newWork.city" required>
    ...
    <div v-if="isEditing">
      ...
      <form @submit.prevent="submitEditWork">
        <input v-model="editWork.name" placeholder="Work Name" disabled/>
        ...
        <select id="city" v-model="editWork.city" required>
          <option value="" disabled>Select your city</option>
          <option v-for="city in cities" :key="city.id" :value="city.name">{{ city.name }}</option>
        </select>
  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        ...
        const responses = await fetch('http://localhost:5000/hello_world');
        if (responses.ok) {
